...
class SegmentTree {
  constructor(nums) {
    ...
    this.segTree = new Array(2 * this.numslength);
    this.build(nums);
  }

  build(nums) {
    let j = 0;
    const N = this.numslength;
    for (let i = N; i < 2 * N; i++) {
  ...
  update(index, val) {
    index += this.numslength;
    this.segTree[index] = val;
    while (index > 1) {
      index = Math.floor(index / 2);
      this.segTree[index] =
        this.segTree[index * 2] + this.segTree[index * 2 + 1];
    }
  }
  ...
  RMinQ = (left, right) => {
    ...
    while (left <= right) {
      if (left % 2 === 1) {
        minElement = Math.min(minElement, this.segTree[left]);
        left++;
      }
      if (right % 2 === 0) {
        minElement = Math.min(minElement, this.segTree[right]);
        right--;
      }
      left = Math.floor(left / 2);
      right = Math.floor(right / 2);
  ...
  RMaxQ = (left, right) => {
    ...
    while (left <= right) {
      if (left % 2 === 1) {
        maxElement = Math.max(maxElement, this.segTree[left]);
        left++;
      }
      if (right % 2 === 0) {
        maxElement = Math.max(maxElement, this.segTree[right]);
        right--;
      }
      left = Math.floor(left / 2);
      right = Math.floor(right / 2);
...
console.log(st.sumRange(2, 4)); //12
st.update(2, 7);
console.log(st.sumRange(2, 4)); //16
console.log(st.RMinQ(2, 4));
console.log(st.RMaxQ(2, 4));
